...
/**
 * Eyes-on-road confirmation (SPEC §6.9): a short buzz plus a tone, with a distinct
 * pattern for undo. Anything that fails here is swallowed — tagging must never block
 * on the vibrator or the audio stack.
 */
class RideFeedback(context: Context) {
    ...
    /** STREAM_MUSIC so it carries over a bar-mount speaker or a BT headset. */
    ...
    private fun playTone(undone: Boolean) {
        ...
        // Mirrors the Garmin field: key click on confirm, reset tone on undo.
        if (undone) {
